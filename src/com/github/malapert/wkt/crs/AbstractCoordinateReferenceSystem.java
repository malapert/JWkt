...
public class AbstractCoordinateReferenceSystem  implements WktDescription, CoordinateReferenceSystem {
    ...
    protected void parseCrs(final WktElt crsWkt) {
        ...
        List<WktElt> attributes = wktEltCollection.getAttributesFor(crsWkt, getKeyword());
        this.setCrsName(attributes.get(0).getKeyword());

        List<WktElt> nodes = wktEltCollection.getNodesFor(crsWkt, getKeyword());
        for (WktElt node : nodes) {
            switch (node.getKeyword()) {                   
                ...
                case Scope.SCOPE_KEYWORD:
                    this.getScopeExtent().setScope(new Scope(crsWkt));
                    break;                    
                ...
                case ExtentFactory.TemporalExtent.TEMPORAL_EXTENT_KEYWORD:
                    Extent extent = ExtentFactory.createFromWkt(node);
                    this.getScopeExtent().getExtentList().add(extent);
                    break;
                default:
    ...
    public Scope getScope() {
        return this.getScopeExtent().getScope();
    }

    @Override
    public List<Extent> getExtentList() {
        return this.getScopeExtent().getExtentList();
    }
