...
public abstract class AbstractDatum implements Datum {
    ...
    protected void parse(final WktElt datumWkt) {
        ...
        List<WktElt> attributes = wktEltCollection.getAttributesFor(datumWkt, this.getKeyword());
        this.setDatumName(attributes.get(0).getKeyword());
        List<WktElt> nodes = wktEltCollection.getNodesFor(datumWkt, this.getKeyword());
    ...
    public static class Anchor implements WktDescription {
        ...
        private void parse(final WktElt datumElts) {
            WktEltCollection wktEltCollection = Singleton.getInstance().getCollection();
            List<WktElt> attributes = wktEltCollection.getAttributesFor(datumElts, KEYWORD_ANCHOR);
