...
public abstract class BaseDerivatedCrsFactory {
    ...
    public static class BaseProjectedCrs implements BaseCrs {
        ...
        private void parse(WktElt datumWkt) {
            ...
            List<WktElt> attributes = wktEltCollection.getAttributesFor(datumWkt, BASE_PROJECTED_CRS);
            this.setBaseCrsName(attributes.get(0).getKeyword());
            List<WktElt> nodes = wktEltCollection.getNodesFor(datumWkt, BASE_PROJECTED_CRS);
        ...
        public StringBuffer toWkt(int deepLevel) {
            ...
            wkt = wkt.append(WKT_SEPARATOR).append("\n").append(Utils.makeSpaces(deepLevel + 1)).append(this.getBaseGeodetic().toWkt(deepLevel + 1));
            wkt = wkt.append(WKT_SEPARATOR).append("\n").append(Utils.makeSpaces(deepLevel + 1)).append(this.getMapProjection().toWkt(deepLevel + 1));
    ...
    public static class BaseEngineeringCrs implements BaseCrs {
        ...
        private void parse(final WktElt baseEngineeringCrsWkt) {
            WktEltCollection wktEltCollection = Singleton.getInstance().getCollection();
            List<WktElt> attributes = wktEltCollection.getAttributesFor(baseEngineeringCrsWkt, BASE_ENGINEERING_CRS_KEYWORD);
            this.setBaseCrsName(attributes.get(0).getKeyword());
            List<WktElt> nodes = wktEltCollection.getNodesFor(baseEngineeringCrsWkt, BASE_ENGINEERING_CRS_KEYWORD);
            for (WktElt node : nodes) {
    ...
    public static class BaseGeodeticCrs implements BaseCrs {
        ...
        private void parse(final WktElt baseGeodeticCrsElts) {
            WktEltCollection wktEltCollection = Singleton.getInstance().getCollection();
            List<WktElt> attributes = wktEltCollection.getAttributesFor(baseGeodeticCrsElts, BASE_GEODETIC_CRS_KEYWORD);
            this.setBaseCrsName(attributes.get(0).getKeyword());
            List<WktElt> nodes = wktEltCollection.getNodesFor(baseGeodeticCrsElts, BASE_GEODETIC_CRS_KEYWORD);
            for (WktElt node : nodes) {
                switch (node.getKeyword()) {
                    case "DATUM":
                    case "GEODETICDATUM":
        ...
        public StringBuffer toWkt(int deepLevel) {
            ...
            if (this.getAngleUnit() != null) {
                wkt = wkt.append(WKT_SEPARATOR).append("\n").append(Utils.makeSpaces(deepLevel + 1)).append(this.getAngleUnit().toWkt(deepLevel + 1));
            }
            wkt = wkt.append("\n").append(Utils.makeSpaces(deepLevel)).append(RIGHT_DELIMITER);
            return wkt;
        }
