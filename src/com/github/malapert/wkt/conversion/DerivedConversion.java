...
public class DerivedConversion extends AbstractConversion {
    ...
    private void parse(final WktElt derivedConversionWkt) {
        ...
        List<WktElt> attributes = wktEltCollection.getAttributesFor(derivedConversionWkt, DERIVED_CONVERSION_KEYWORD);
        setName(attributes.get(0).getKeyword());

        List<WktElt> nodes = wktEltCollection.getNodesFor(derivedConversionWkt, DERIVED_CONVERSION_KEYWORD);
    ...
    public StringBuffer toWkt(int deepLevel) {
        ...
        if (!getParameters().isEmpty()) {
            for(Operation parameter:getParameters()) {
